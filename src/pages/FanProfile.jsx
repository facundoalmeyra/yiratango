...
function EditFanProfileForm({ user, fanProfile, onSave, onCancel, onDelete, isSaving }) {
  ...
  const [formData, setFormData] = useState({
    name: fanProfile?.name || user.full_name || '',
    avatar_url: fanProfile?.avatar_url || user.avatar_url || user.picture || '',
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      // Auto-save the avatar if fanProfile exists
      if (fanProfile?.id) {
        await base44.entities.Fan.update(fanProfile.id, { avatar_url: file_url });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-3 justify-end pt-4 border-t border-white/10">
        <Button variant="ghost" onClick={onCancel} className="text-white/80 hover:text-white">
          {t('cancel')}
        </Button>
        <Button 
...
function FollowedArtistCard({ artist, user, initialFollowRecord, index }) {
  ...
  const handleConfirmUnfollow = async (e) => {
    ...
    try {
      ...
      toast.error(t('failedToUnfollow'));
    } finally {
      setIsProcessing(false);
    }
  };

  const handleCancelUnfollow = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setShowConfirm(false);
  };
  ...
  return (
    ...
    >
      <Link
        ...
        className="group p-4 hover:bg-white/5 transition-colors block"
      >
              ...
              <div className="flex items-center gap-3 text-sm text-white/70">
                ...
                {artist.current_city && (
                  <span className="flex items-center gap-1">
                    <MapPin className="w-3 h-3" /> {artist.current_city}
                  </span>
                )}
              </div>
            </div>
            ...
            <Button
              ...
              className={`w-full sm:w-auto px-4 rounded-full h-9 text-xs md:text-sm font-semibold transition-all shadow-lg flex items-center justify-center gap-2 whitespace-nowrap ${
                isFollowing
                  ...
                  : 'bg-white border-0 text-black hover:bg-gray-200'
              }`}
            >
              {isProcessing ? (
      ...
      <AnimatePresence>
        {showConfirm && (
          ...
          >
            <div 
              className="bg-[#111111] border border-white/10 rounded-2xl max-w-sm w-full p-6 text-center"
...
export default function FanProfile() {
  ...
  const { data: fans = [], isLoading: loadingFans } = useQuery({
    queryKey: ['fans', user?.email || user?.id],
  ...
  const { data: follows = [], isLoading: loadingFollows } = useQuery({
    queryKey: ['follows', user?.email || user?.id],
    queryFn: () => base44.entities.Follow.filter({ fan_user_id: user?.email || user?.id }),
    enabled: !!(user?.email || user?.id),
  });

  // For organizers: fetch their own sent requests. Must be called unconditionally (Rules of Hooks).
  const { data: myRequests = [] } = useQuery({
    queryKey: ['my_sent_requests', user?.email || user?.id],
    queryFn: () => base44.entities.VisitRequest.filter({ fan_user_id: user?.email || user?.id }, '-created_date'),
    enabled: !!(user?.email || user?.id),
  });
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (data) => {
      ...
      if (fanRecord) {
        await base44.entities.Fan.update(fanRecord.id, {
          name: data.name,
          avatar_url: data.avatar_url,
          city: data.city,
          latitude: data.latitude,
          longitude: data.longitude,
          discovery_radius: data.discovery_radius
        });
      }
      
      return base44.auth.me(); // Refresh user
    },
  ...
  const deleteFanMutation = useMutation({
    mutationFn: async () => {
      ...
      // Delete all followed artists relations
      ...
      // Delete Visit Requests made by this fan
      ...
      // Delete Artist and associated records if they exist
      const artistRecords = await base44.entities.Artist.filter({ created_by: userEmail });
      for (const artist of artistRecords) {
        const artistTours = await base44.entities.Tour.filter({ artist_id: artist.id });
        await Promise.all(artistTours.map(t => base44.entities.Tour.delete(t.id)));
        
        const artistRequests = await base44.entities.VisitRequest.filter({ artist_id: artist.id });
        await Promise.all(artistRequests.map(r => base44.entities.VisitRequest.delete(r.id)));

        const artistFollows = await base44.entities.Follow.filter({ artist_id: artist.id });
        await Promise.all(artistFollows.map(f => base44.entities.Follow.delete(f.id)));

        const artistNotifications = await base44.entities.Notification.filter({ artist_id: artist.id });
        await Promise.all(artistNotifications.map(n => base44.entities.Notification.delete(n.id)));

        await base44.entities.Artist.delete(artist.id);
      }
  ...
  useEffect(() => {
    if (!loadingUser && !loadingFans && user && fans.length === 0) {
      base44.auth.updateMe({ role: 'user' }).finally(() => {
        base44.auth.logout();
      });
    }
  }, [loadingUser, loadingFans, user, fans]);
  ...
  const followedArtists = follows
    .map(f => {
      ...
      return artist ? { artist, followRecord: f } : null;
    })
    .filter(Boolean);

  const fanProfile = fans[0] || null;
  const isOrganizer = fanProfile?.role_type === 'organizer';
  const displayName = fanProfile?.name || user.full_name || 'Tango Fan';
  const displayAvatar = fanProfile?.avatar_url || user.avatar_url || user.picture;

  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white/20">
      <SEO 
        title={`${displayName} - Profile Settings`}
        description="Manage your fan profile and followed artists."
      />
        ...
        <section className="mb-6 md:mb-8 flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-8 pt-4">
          ...
          >
            <div className="relative z-10 w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-[#1A1A1A] shadow-2xl bg-[#111111] transition-all duration-300 ring-1 ring-white/10">
              <UserAvatar 
                user={{ ...user, avatar_url: displayAvatar, full_name: displayName }} 
                size="full" 
                className="w-full h-full text-4xl text-white bg-transparent"
              />
            </div>
          </motion.div>
              ...
              {followedArtists.length === 0 ? (
                ...
                followedArtists.map((item, index) => (
                  <FollowedArtistCard 
                    ...
                    artist={item.artist} 
                    user={user} 
                    initialFollowRecord={item.followRecord}
                    index={index} 
                  />
                ))
              )}
