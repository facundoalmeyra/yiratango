...
export default function ProfileSettings() {
  ...
  const profileStructuredData = artistProfile ? {
    ...
    "jobTitle": "Artist",
    "image": artistProfile.avatar_url || "https://qtrypzzcjebvfcihiynt.supabase.co/storage/v1/object/public/base44-prod/public/6985f5bb902ec2f8c9596a0d/d74c463db_favicon2.png",
  ...
  const { data: dates = [], refetch: refetchDates, isLoading: loadingDates } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch by both ID and slug to handle tours imported with slug as artist_id
      const [byId, bySlug] = await Promise.all([
        base44.entities.Tour.filter({ artist_id: artistProfile.id }),
        artistProfile.slug ? base44.entities.Tour.filter({ artist_id: artistProfile.slug }) : Promise.resolve([]),
      ]);
  ...
  // Short ID generation utility (5 chars alphanumeric)
  ...
  const updateArtistMutation = useMutation({
    ...
    onSuccess: (updatedData, variables) => {
      ...
      // Only show toast if it wasn't a silent update (like auto-slug)
  ...
  const deleteArtistMutation = useMutation({
    mutationFn: async (id) => {
      ...
      // 1. Delete the specific Artist and associated records
      const artistTours = await base44.entities.Tour.filter({ artist_id: id });
      await Promise.all(artistTours.map(t => base44.entities.Tour.delete(t.id)));
      
      const artistRequests = await base44.entities.VisitRequest.filter({ artist_id: id });
      await Promise.all(artistRequests.map(r => base44.entities.VisitRequest.delete(r.id)));

      const artistFollows = await base44.entities.Follow.filter({ artist_id: id });
      await Promise.all(artistFollows.map(f => base44.entities.Follow.delete(f.id)));

      const artistNotifications = await base44.entities.Notification.filter({ artist_id: id });
      await Promise.all(artistNotifications.map(n => base44.entities.Notification.delete(n.id)));

      await base44.entities.Artist.delete(id);

      // 2. Delete Fan and associated records (if they have them)
      const fanRecords = await base44.entities.Fan.filter({ user_id: userEmail });
      await Promise.all(fanRecords.map(f => base44.entities.Fan.delete(f.id)));

      const followRecords = await base44.entities.Follow.filter({ fan_user_id: userEmail });
      await Promise.all(followRecords.map(f => base44.entities.Follow.delete(f.id)));

      const fanRequests = await base44.entities.VisitRequest.filter({ fan_user_id: userEmail });
      await Promise.all(fanRequests.map(r => base44.entities.VisitRequest.delete(r.id)));

      const fanNotifications = await base44.entities.Notification.filter({ fan_user_id: userEmail });
      await Promise.all(fanNotifications.map(n => base44.entities.Notification.delete(n.id)));

      // Reset the role and wipe user data
      await base44.auth.updateMe({ role: 'user', full_name: '', avatar_url: '' });
    },
    onSuccess: async () => {
      toast.success(t('accountDeleted'));
      await base44.auth.logout(createPageUrl('Map'));
    },
  });
  ...
  useEffect(() => {
    if (user && !loadingArtists) {
      ...
      const profile = artists.find(m => 
        m.created_by === userIdentifier || 
        m.claimed_by_user_id === userIdentifier
      );
  ...
  const createDateMutation = useMutation({
    mutationFn: (data) => base44.entities.Tour.create(data),
    onSuccess: () => {
      refetchDates();
  ...
  if (!user) {
    if (typeof window !== 'undefined') window.location.href = createPageUrl('Map');
    return null;
  }
  ...
  const activeMode = tourState.status === 'LIVE' ? 'current' 
                   : tourState.status === 'TRANSIT' ? 'transit'
                   : 'manual'; // Default to manual/home if not LIVE or TRANSIT
  
  const currentTour = tourState.status === 'LIVE' ? tourState.tour : null;
  // If in transit, we show next tour info. If home, we might show next tour if available but mode is manual/home
  const nextTour = tourState.status === 'TRANSIT' ? tourState.nextTour : tourState.nextTour;
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white/20">
      ...
      {/* Header - Sticky only on desktop, relative on mobile to hide when searching */}
      <header className="relative md:sticky md:top-0 z-40 bg-black/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link
            ...
            onClick={() => {
              // Pre-invalidate queries to speed up the map load
              queryClient.invalidateQueries({ queryKey: ['artists'] });
              queryClient.invalidateQueries({ queryKey: ['dates'] });
            }}
      ...
      <main className="max-w-4xl mx-auto px-6 py-2 md:py-8 pb-24">
            ...
            >
              ...
              >
                <UserAvatar 
                  ...
                  size="full" 
                  className="w-full h-full text-2xl text-black bg-white"
                />
              </div>
            </motion.div>
            ...
            >
              <h1 className="text-xl md:text-3xl font-bold tracking-tight text-white leading-tight">
                {(!artistProfile.category || artistProfile.category === 'Maestro') && artistProfile.profileType === 'Couple' && artistProfile.partner_name 
                  ? `${artistProfile.name} & ${artistProfile.partner_name}` 
                  : artistProfile.name}
              </h1>
            </motion.div>
          </div>

          {/* Shareable Link and Actions */}
        ...
        {/* Add Dates Banner - Full Width */}
         {(() => {
            const now = new Date(); now.setHours(0,0,0,0);
            const futureDates = dates.filter(t => {
              const end = new Date(t.end_date || t.date); end.setHours(0,0,0,0);
              return end >= now;
            });
        ...
        {/* Status & Location Card - Only show when has future tours */}
        {(() => {
          const now = new Date(); now.setHours(0,0,0,0);
          const futureDates = dates.filter(t => {
            const end = new Date(t.end_date || t.date); end.setHours(0,0,0,0);
        ...
        >
          <div className="rounded-2xl relative bg-[#111111] border border-white/5 shadow-2xl min-h-[120px] md:min-h-[160px] flex items-center">
            {/* Background Map - Full Container Height */}
            <div className="absolute inset-0 z-0 opacity-60 overflow-hidden rounded-2xl">
              <div className="w-full h-full relative">
                ...
                <MiniMap 
                latitude={
                  activeMode === 'current' ? currentTour?.latitude : 
                  activeMode === 'transit' ? lastTour?.latitude :
                  activeMode === 'next' ? nextTour?.latitude : 
                  artistProfile.current_latitude
                }
                longitude={
                  activeMode === 'current' ? currentTour?.longitude : 
                  activeMode === 'transit' ? lastTour?.longitude :
                  activeMode === 'next' ? nextTour?.longitude : 
                  artistProfile.current_longitude
                }
                markerColor={activeMode === 'current' ? '#ffffff' : '#ffffff'}
                ...
                <div className="flex flex-col items-center md:items-start space-y-1.5 md:space-y-2 flex-1">
                   <div className="inline-flex items-center justify-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#00C2D4]/10 text-[#00C2D4] border border-[#00C2D4]/20 mb-1">
                     <span className="relative flex h-1.5 w-1.5">
                       <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#00C2D4] opacity-75"></span>
                       <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[#00C2D4] shadow-[0_0_8px_rgba(0,194,212,0.5)]"></span>
                     </span>
                   ...
                   <p className="text-white/70 flex items-center justify-center md:justify-start gap-1.5 text-xs md:text-sm font-light">
                     ...
                     {t('until')} {format(new Date(currentTour.end_date || currentTour.date), "d 'de' MMMM", { locale: dateLocale })}
                   </p>
                </div>
                <Link 
                   to={createPageUrl('Map')}
                   className="flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors font-['JetBrains_Mono',_monospace] font-extrabold text-[12px] uppercase text-white border border-white/10"
                 >
                ...
                <div className="flex flex-col items-center md:items-start space-y-1.5 md:space-y-2 flex-1">
                   <div className="inline-flex items-center justify-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#F2EF1D]/10 text-[#F2EF1D] border border-[#F2EF1D]/20 mb-1">
                     <span className="relative flex h-1.5 w-1.5">
                       <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#F2EF1D] opacity-75"></span>
                       <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-[#F2EF1D] shadow-[0_0_8px_rgba(242,239,29,0.5)]"></span>
                     </span>
                   ...
                   <p className="text-white/70 flex items-center justify-center md:justify-start gap-1.5 text-xs md:text-sm font-light">
                     ...
                     {t('nextStopStarts')} {format(new Date(nextTour.start_date || nextTour.date), "d 'de' MMMM", { locale: dateLocale })}
                   </p>
                </div>
                <Link 
                  to={`${createPageUrl('Map')}?zoomToUser=true`}
                  className="flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors font-['JetBrains_Mono',_monospace] font-extrabold text-[12px] uppercase text-white border border-white/10"
                >
                  <Map className="w-4 h-4" />
                  {t('viewOnMap')}
                ...
                <button
                  ...
                  className="w-full text-left flex flex-col items-center md:items-start space-y-1.5 md:space-y-2 hover:opacity-80 transition-opacity"
                >
                  {(() => {
                    const now = new Date(); now.setHours(0,0,0,0);
                    const futureDates = dates.filter(t => {
        ...
        <div className="flex w-full mb-8 border-b border-white/10">
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-4 text-sm md:text-base font-bold transition-all relative ${
              activeTab === 'dates' 
                ? 'text-white' 
              ...
              {dates.length === 0 ? (
                ...
                [...dates]
                  ...
                  .map((date, index) => {
                    ...
                    const endDate = new Date(date.end_date || date.date);
        ...
        <TourFormModal 
          ...
          onSubmit={(data) => {
            if (editingDate) {
              updateDateMutation.mutate({ id: editingDate.id, data });
            } else {
              createDateMutation.mutate({ ...data, artist_id: artistProfile.id });
            }
          }}
          onCreated={() => {
            setShowDateForm(false);
        ...
        {/* Delete Confirmation Modal */}
         <AnimatePresence>
           {dateToDelete && (
             ...
             >
               <div 
                 className="bg-[#111111] border border-white/10 rounded-2xl max-w-sm w-full p-6"
                 onClick={(e) => e.stopPropagation()}
               >
                 <h3 className="text-lg font-bold text-white mb-2">{t('removeDateTitle')}</h3>
                 <p className="text-white/80 mb-6 text-sm">
                   {t('removeDateDesc')} {dateToDelete.city} {t('removeDateDesc2')}
                 </p>
                 <div className="flex gap-3 justify-end">
                   ...
                   <Button
                     onClick={() => deleteDateMutation.mutate(dateToDelete.id)}
